...
> 主题范围：workflow 与 agent 辨析、ReAct 循环与 CoT/P&E 模式对比、规划与思考、asyncio 并发模型、function calling 链路、工具 docstring 规范、MCP 接口与协议选型。
> 对应实现：`src/finrag/agent_graph.py`。
> 结构：四个主题分组，组内条目按学习递进排列（相关条目相邻）。
...
### Q3 · CoT、Plan-and-Execute、ReAct 的区别？
...
**身边的活例子**：opencode 的 todowrite 工具就是外置规划——干多步任务前先列清单逐项勾销（规划），每一步怎么改是思考。**P&E 把两者拆给不同角色（planner/executor），ReAct 只有思考没有规划**——这正是 Q3 说"ReAct 容易绕弯"的根源：没有规划兜底，每步的局部最优走偏了才发现。
...
判据是瓶颈类型：工具全是 **IO 密集**（调 LLM API、读文件）——单循环已把等待全部重叠（Q5 收益拿满），没有"算"的任务可分给其他核。只有出现 **CPU 密集**工具（本地 embedding 计算、解析超大 PDF、复杂量化回测）才值得引进程池。
...
### Q7 · Function calling 全链路——项目是怎么实现工具调用的？
...
关联 → Q8（工具描述规范）、Q9/Q10（工具如何从进程内走向进程外与标准化）；本项目的进程内实现在 `src/finrag/tools.py`。

---

### Q8 · 工具写法规范——为什么 docstring 即接口文档？
...
**原理**（详见 Q7 的 bind_tools 链路）：`@tool` 装饰器把函数 docstring 全文透传为请求体的 `description` 字段。docstring 对 Python 是注释，对 tool calling 是**发给模型的接口文档**。
...
关联 → Q7（bind_tools 四步链路）、[评估体系.md](评估体系.md) Q3（同样的"透传给模型"原理在 judge 输出契约上的复用）

---

## 四、MCP 与工具标准化

### Q9 · 基于 CRUD 企业级业务系统怎么开发 MCP 接口？
...
关联 → Q10（为什么需要 MCP 的协议经济学）

---

### Q10 · 为什么需要 MCP？让 LLM 直接走 REST 不就好了？
...
workflow vs agent 判据（Q1）→ ReAct 循环节拍器（Q2）→ CoT/P&E/ReAct 三模式对比（Q3）→ 规划 vs 思考（Q4）→ 循环内工具并发 gather（Q5）→ 并发 vs 并行的边界（Q6）→ function calling 四步链路（Q7）→ 工具 docstring 即接口文档（Q8）→ 工具进程外化与 MCP 接口设计（Q9）→ MCP vs REST 的标准化经济学（Q10）。
